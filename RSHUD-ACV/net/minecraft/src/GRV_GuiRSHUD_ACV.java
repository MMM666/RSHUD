...
public class GRV_GuiRSHUD_ACV extends GRH_GuiRSHUDConfigure {
	...
	protected MMM_GuiSlider[] colormarker;
	protected MMM_GuiSlider colorlowgain;
	...
	public GRV_GuiRSHUD_ACV(BaseMod basemod) {
		...
		colormarker = new MMM_GuiSlider[4];
		lowGain = 0.3F;
		isStatus = true;
	...
	public void initGui() {
		...
		for (int i = 0; i < 4; i++) {
			colornormal[i] = new MMM_GuiSlider(10 + i, hwSize - 160,
					hhSize + 18 + (20 * i), str[i], f[i]).setDisplayString();
			buttonList.add(colornormal[i]);
		}
		colorlowgain = new MMM_GuiSlider(14, hwSize - 160,
				hhSize + 18 + 80, "low Gain", lowGain).setDisplayString();
		buttonList.add(colorlowgain);
		...
		for (int i = 0; i < 4; i++) {
			coloralert[i] = new MMM_GuiSlider(30 + i, hwSize + 60, hhSize
					+ 18 + (20 * i), str[i], f[i]).setDisplayString();
			buttonList.add(coloralert[i]);
		}
		f = getRGBA(ColorInt_Maker);
		for (int i = 0; i < 4; i++) {
			colormarker[i] = new MMM_GuiSlider(30 + i, hwSize + 60, hhSize
					- 98 + (20 * i), str[i], f[i]).setDisplayString();
			buttonList.add(colormarker[i]);
		}
		buttonList.add(new GuiButton(300, hwSize - 50, hhSize - 106, 100, 20,
				String.format("Value Size x%.1f", textSize)));
		buttonList.add(new GuiButton(310, hwSize - 160, hhSize - 38, 100, 20,
				isStatus ? "Status ON" : "Status OFF"));
	}

	@Override
	public void drawDefaultBackground() {
		drawGradientRect(hwSize + 50, hhSize - 116, hwSize + 170, hhSize - 96,
				0xd0101010, 0xc0101010);
		drawGradientRect(hwSize - 170, hhSize, hwSize + 170, hhSize + 20,
	...
	protected void actionPerformed(GuiButton guibutton) {
		...
		if (guibutton.id == 300) {
			...
			guibutton.displayString = String.format("Value Size x%.1f",
					textSize);
	...
	public void drawScreen(int i, int j, float f) {
		...
		ColorInt_Normal = setRGBA(colornormal[0].getSliderValue(),
				colornormal[1].getSliderValue(),
				colornormal[2].getSliderValue(),
				colornormal[3].getSliderValue());
		ColorInt_Warning = setRGBA(colorwarning[0].getSliderValue(),
				colorwarning[1].getSliderValue(),
				colorwarning[2].getSliderValue(),
				colorwarning[3].getSliderValue());
		ColorInt_Alert = setRGBA(coloralert[0].getSliderValue(),
				coloralert[1].getSliderValue(), coloralert[2].getSliderValue(),
				coloralert[3].getSliderValue());
		ColorInt_Maker = setRGBA(colormarker[0].getSliderValue(),
		...
		mod_GRV_RSHUD_ACV.Color_Warning = String.format("%08x",
				ColorInt_Warning);
		...
		String s = "NORMAL";
		drawString(fontRenderer, s,
				hwSize - 110 - fontRenderer.getStringWidth(s) / 2, hhSize + 6,
				ColorInt_Normal);
		s = "WARNING";
		drawString(fontRenderer, s,
				hwSize - fontRenderer.getStringWidth(s) / 2, hhSize + 6,
				ColorInt_Warning);
		s = "ALERT";
		drawString(fontRenderer, s,
				hwSize + 110 - fontRenderer.getStringWidth(s) / 2, hhSize + 6,
		...
		s = "MARKER";
		drawString(fontRenderer, s,
				hwSize + 110 - fontRenderer.getStringWidth(s) / 2,
				hhSize - 110, ColorInt_Maker);

		// サンプル表示用
		GL11.glEnable(GL11.GL_BLEND);
		s = "MARKER";
		fontRenderer.drawString(s, hwSize - fontRenderer.getStringWidth(s) / 2,
				hhSize - 75, ColorInt_Maker);
		s = "NORMAL";
		fontRenderer.drawString(s, hwSize - fontRenderer.getStringWidth(s) / 2,
				hhSize - 45, ColorInt_Normal);
		s = "LOW GAIN";
		fontRenderer.drawString(s, hwSize - fontRenderer.getStringWidth(s) / 2,
				hhSize - 35, getColorAmp(ColorInt_Normal, lowGain));
		s = "WARNING";
		fontRenderer.drawString(s, hwSize - fontRenderer.getStringWidth(s) / 2,
				hhSize - 25, ColorInt_Warning);
		s = "ALERT";
		fontRenderer.drawString(s, hwSize - fontRenderer.getStringWidth(s) / 2,
				hhSize - 15, ColorInt_Alert);
	...
	public void renderRSHUD(Minecraft mc, int i, int j) {
		...
		// Route
		// ルートの判定
		if (entityRoute == null || entityRoute.isDead
				|| entityRoute.worldObj != mc.theWorld) {
		...
		int valFoodTimer = 0;
		try {
			valFoodExh = (Float) (ModLoader.getPrivateValue(FoodStats.class,
					mcpl.foodStats, 2)); // 4
			valFoodTimer = (Integer) (ModLoader.getPrivateValue(
					FoodStats.class, mcpl.foodStats, 3)); // 80
		} catch (Exception e) {
		}

		// RECON
		List lre = (new ArrayList());
		lre.addAll(viewRECONEntitys);
		// if (RenderManager.instance.renderEngine != null &&
		// !viewRECONEntitys.isEmpty()) {
		if (entityRoute != null && !entityRoute.isDead && !lre.isEmpty()) {
			...
			if (lrm.options != null) {
				...
				Iterator literator1 = lre.iterator();
				while (literator1.hasNext()) {
					...
					if (lentity1 != null && !(lentity1 == mcpl)) {
						...
						GL11.glPushMatrix();
						GL11.glTranslated(lentity1.lastTickPosX
								- lrm.renderPosX, lentity1.lastTickPosY
								- lrm.renderPosY, lentity1.lastTickPosZ
								- lrm.renderPosZ);
						...
						lrm.renderEntityWithPosYaw(lentity1,
								lentity1.lastTickPosX - lrm.renderPosX,
								lentity1.lastTickPosY - lrm.renderPosY,
								lentity1.lastTickPosZ - lrm.renderPosZ,
								lentity1.prevRotationYaw, 0F);
		...
		// EN
		float deg;
		float valEN = ((float) mcpl.foodStats.getFoodLevel()
				+ mcpl.foodStats.getSaturationLevel() - (valFoodExh / 4F)) / 40F;
		...
		if (itemstack != null && (containsAmmo(itemstack) || itemstack.isStackable())) {
			...
			if (ll != null && maxammo <= 0) {
				for (ItemStack lis : ll) {
					if (lis.getItem() != null) {
						maxammo = lis.getItem().maxStackSize;
					}
				}
			}
			...
			drawMeter(tessellator, r0 + 2D, r0 + 3D, 272, 360 - d1,
					Math.min(1.0F, maxammo));
			// 枠線
			r1 = r0 + 1.0D;
		...
		float valSpeed = MathHelper.sqrt_double(lvx * lvx + lvy * lvy + lvz
				* lvz) * 1000F;
		...
		for (int li = 0; li < 4; li++) {
			...
			if (itemarmor != null) {
				valArmor[li] = 1F - (float) itemarmor.getItemDamage()
						/ (float) itemarmor.getMaxDamage();
				valAC += valArmor[li];
				...
				valACAll += ll;
				colArmor[li] = getColorAmp(
						getArmorColor(itemarmor),
						Math.max(lowGain,
								Math.min(1.0F, messageAlpha[ma_armor1 + li])));
			} else {
		...
		if (entityRoute != null && isRouteView) {
			...
			setGLTextColor(ColorInt_Maker, 1.0F);
			for (int li = 0; li < 3 && entityRoute.routePos[li] != null
					&& entityRoute.markerPos[li] != null; li++) {
				float lfz = entityRoute.markerPos[li].get(2);
				if (lfz >= 0.0F && lfz <= 1.0F) {
					// クリッピング範囲内
					flag[li] = true;
					GL11.glPushMatrix();
					GL11.glTranslatef(entityRoute.markerPos[li].get(0) / zoomx,
							j - entityRoute.markerPos[li].get(1) / zoomy, 0.0F);
					...
					String sdis = String.format(
							"%.2f",
							MathHelper.sqrt_double(ld0 * ld0 + ld1 * ld1 + ld2
									* ld2));
					mc.fontRenderer.drawString(sdis,
							mc.fontRenderer.getStringWidth("0.00")
									- mc.fontRenderer.getStringWidth(sdis),
							(int) (10F / textSize), ColorInt_Maker);
					sdis = "ROUTE";
					mc.fontRenderer
							.drawString(
									sdis,
									-mc.fontRenderer.getStringWidth(sdis) / 2,
									(int) (-10.5F / textSize - mc.fontRenderer.FONT_HEIGHT
											* textSize), ColorInt_Maker);
			...
			if (flag[0]) {
				...
				f = (f > 1.0F) ? -1 : 1;
				tessellator.addVertex(entityRoute.playerPos.get(0) / zoomx, j
						- entityRoute.playerPos.get(1) * f / zoomy, 0D);
				tessellator.addVertex(entityRoute.markerPos[0].get(0) / zoomx,
						j - entityRoute.markerPos[0].get(1) / zoomy, 0D);
			}
			if (flag[0] && flag[1]) {
				tessellator.addVertex(entityRoute.markerPos[0].get(0) / zoomx,
						j - entityRoute.markerPos[0].get(1) / zoomy, 0D);
				tessellator.addVertex(entityRoute.markerPos[1].get(0) / zoomx,
						j - entityRoute.markerPos[1].get(1) / zoomy, 0D);
			}
			if (flag[1] && flag[2]) {
				tessellator.addVertex(entityRoute.markerPos[1].get(0) / zoomx,
						j - entityRoute.markerPos[1].get(1) / zoomy, 0D);
				tessellator.addVertex(entityRoute.markerPos[2].get(0) / zoomx,
						j - entityRoute.markerPos[2].get(1) / zoomy, 0D);
			}
			tessellator.draw();
		}
		...
		ldx = centerX + r0 + 3D;
		ldy = centerY - 1D;
		mc.fontRenderer.drawString(s, (int) (ldx / textSize),
				(int) (ldy / textSize) - mc.fontRenderer.FONT_HEIGHT, colEN);
		// AP
		s = String.format("%06.2f", valAP * 100F);
		ldx = centerX - r0 - 1D;
		ldy = centerY - 1D;
		mc.fontRenderer.drawString(s,
				(int) (ldx / textSize) - mc.fontRenderer.getStringWidth(s),
				(int) (ldy / textSize) - mc.fontRenderer.FONT_HEIGHT, colAP);
		// WEAPON
		if (itemstack != null) {
			...
			ldx = centerX + r0 - 4D;
			ldy = centerY;
			txtcolor = getColorAmp(colWep,
					Math.max(lowGain, Math.min(1.0F, messageAlpha[ma_weapon])));
			mc.fontRenderer.drawString(s, (int) (ldx / textSize - 6D)
					- mc.fontRenderer.getStringWidth(s), (int) (ldy / textSize)
					- mc.fontRenderer.FONT_HEIGHT * 3, txtcolor);
		} else {
		...
		if (valAmmo > 0) {
			...
			ldx = centerX + r0 + 5D;
			ldy = centerY;
			txtcolor = getColorAmp(colAmmo,
					Math.max(lowGain, Math.min(1.0F, messageAlpha[ma_ammo])));
			mc.fontRenderer.drawString(s, (int) (ldx / textSize),
					(int) (ldy / textSize) - mc.fontRenderer.FONT_HEIGHT * 3,
					txtcolor);
		} else {
		...
		ldx = centerX + r0 - 4D;
		ldy = centerY;
		txtcolor = getColorAmp(colSpeed,
				Math.max(lowGain, Math.min(1.0F, messageAlpha[ma_speed])));
		mc.fontRenderer.drawString(s, (int) (ldx / textSize - 6D)
				- mc.fontRenderer.getStringWidth(s), (int) (ldy / textSize)
				+ mc.fontRenderer.FONT_HEIGHT * 2, txtcolor);
		// HEIGHT
		...
		ldx = centerX - r0 + 4D;
		ldy = centerY;
		txtcolor = getColorAmp(colHeight,
				Math.max(lowGain, Math.min(1.0F, messageAlpha[ma_height])));
		mc.fontRenderer.drawString(s, (int) (ldx / textSize + 6D),
				(int) (ldy / textSize) + mc.fontRenderer.FONT_HEIGHT * 2,
				txtcolor);
		// AC
		if (valAC > 0F) {
			...
			ldx = centerX - r0 + 4D;
			ldy = centerY;
			txtcolor = getColorAmp(colAC,
					Math.max(lowGain, Math.min(1.0F, messageAlpha[ma_armor])));
			mc.fontRenderer.drawString(s, (int) (ldx / textSize + 6D),
					(int) (ldy / textSize) - mc.fontRenderer.FONT_HEIGHT * 3,
					txtcolor);
		}
		...
		if (valArmor[0] >= 0) {
			...
			ldx = centerX - r0 - 15D;
			ldy = centerY;
			mc.fontRenderer.drawString(s, (int) (ldx / textSize + 26D)
					- mc.fontRenderer.getStringWidth(s), (int) (ldy / textSize)
					- mc.fontRenderer.FONT_HEIGHT * 6, colArmor[0]);
		}
		if (valArmor[1] >= 0) {
			s = String.valueOf(messageLastValue[ma_armor2]);
			ldx = centerX - r0 - 11D;
			ldy = centerY;
			mc.fontRenderer.drawString(s, (int) (ldx / textSize + 16D)
					- mc.fontRenderer.getStringWidth(s), (int) (ldy / textSize)
					- mc.fontRenderer.FONT_HEIGHT * 5, colArmor[1]);
		}
		if (valArmor[2] >= 0) {
			s = String.valueOf(messageLastValue[ma_armor3]);
			ldx = centerX - r0 - 8D;
			ldy = centerY;
			mc.fontRenderer.drawString(s, (int) (ldx / textSize + 7D)
					- mc.fontRenderer.getStringWidth(s), (int) (ldy / textSize)
					- mc.fontRenderer.FONT_HEIGHT * 4, colArmor[2]);
		}
		if (valArmor[3] >= 0) {
			...
			mc.fontRenderer.drawString(s, (int) (ldx / textSize)
					- mc.fontRenderer.getStringWidth(s), (int) (ldy / textSize)
					- mc.fontRenderer.FONT_HEIGHT * 3, colArmor[3]);
		}
		//
		GL11.glPopMatrix();
		...
		if (valMes != null) {
			...
			ldy = centerY + 25D;
			GL11.glEnable(GL11.GL_TEXTURE_2D);
			mc.fontRenderer.drawString(valMes, (int) ldx - widMes + 1,
					(int) ldy, getColorAmp(colMes, iblink ? 0.3F : 1.0F));
			// 枠
			widMes += 1;
			GL11.glLineWidth(1.0F);
			setGLTextColor(ColorInt_Normal, iblink ? 0.3F : 1.0F);
			drawOutline(tessellator, ldy - 1D, ldx - widMes, ldy
					+ mc.fontRenderer.FONT_HEIGHT, ldx + widMes, 2D);
		}
		...
		if (mcpl.isSprinting() || mcpl.isJumping) {
			...
			mc.fontRenderer.drawString(s, (int) ldx + 1 - widMes, (int) ldy,
					getColorAmp(ColorInt_Alert, 0.5F));
			setGLTextColor(ColorInt_Normal, 1.0F);
			widMes += 1;
			GL11.glLineWidth(1.0F);
			drawOutline(tessellator, ldy - 1D, ldx - widMes, ldy
		...
		if (counterMessage2 >= mc.theWorld.getWorldTime()) {
			...
			int widMes = mc.fontRenderer.getStringWidth(s) / 2;
			mc.fontRenderer.drawString(s, (int) (1 - widMes), 0,
					getColorAmp(ColorInt_Normal, iblink ? 1.0F : 0.3F));
			widMes += 1;
			GL11.glLineWidth(1.0F);
			drawOutline(tessellator, -1D, -widMes, mc.fontRenderer.FONT_HEIGHT,
					widMes, 1D);
			GL11.glPopMatrix();
		}

		if (isStatus && mc.gameSettings != null) {
			// STATUS
			...
			// TODO:状態効果
			for (Iterator iterator = mcpl.getActivePotionEffects().iterator(); iterator
					.hasNext();) {
				...
				GL11.glPushMatrix();
				GL11.glTranslatef((40F / var5.getScaleFactor() + lx
						* (var6 / 8)),
						var7 + (140F - lx * 80F) / var5.getScaleFactor()
								- (lpid >>> 2) * 50F, 0F);
				// GL11.glTranslatef(var6 + 20F + lx * 30F, +
				// (float)mc.displayHeight - 80F - lx * 40F - (lpid >>> 2) *
				// 50F, 0F);
				// 書き文字
				int lc = getColorAmp(potion.isBadEffect() ? ColorInt_Alert
						: ColorInt_Normal, 0.8F);
				int widMes = mc.fontRenderer.getStringWidth(s) / 2;
			...
			if (itemstack != null && itemstack.hasTagCompound()) {
				...
				if (nbttaglist != null) {
					...
					for (int li = 0; li < nbttaglist.tagCount(); li++) {
						short word0 = ((NBTTagCompound) nbttaglist.tagAt(li))
								.getShort("id");
						short word1 = ((NBTTagCompound) nbttaglist.tagAt(li))
								.getShort("lvl");
						// short word1 = 1;
						if (Enchantment.enchantmentsList[word0] != null) {
							s = Enchantment.enchantmentsList[word0]
									.getTranslatedName(word1);
							float lx = (float) (word0 & 0x07);
							float ly = (float) (word0 >>> 3);
							GL11.glPushMatrix();
							GL11.glTranslatef((float) mc.displayWidth - 148F
									- ly * 16F, (float) mc.displayHeight - 80F
									- ly * 40F - lx * 50F, 0F);
							// 書き文字
							int widMes = mc.fontRenderer.getStringWidth(s) / 2;
	...
	private void drawXross(Tessellator tessellator, double ldx, double ldy,
			double r1, double r2, double wid) {
		r2 += r1;
	...
	private void drawOutline(Tessellator tessellator, double top, double left,
			double bottom, double right, double length) {
		GL11.glDisable(GL11.GL_TEXTURE_2D);
		tessellator.startDrawing(GL11.GL_LINES);
	...
	private void drawMeter(Tessellator tessellator, double r1, double r2,
			float degStart, float degEnd, float range) {
		if (range <= 0.0F)
			return;
		...
		if (vec > 0) {
			for (li = 0; li < vec; li += 2) {
				...
				ldx = MathHelper.cos(deg);
				ldy = MathHelper.sin(deg);
				tessellator.addVertex(ldx * r2 + centerX, ldy * r2 + centerY,
						0.0D);
				tessellator.addVertex(ldx * r1 + centerX, ldy * r1 + centerY,
						0.0D);
			}
	...
	public void setStartRoute(EntityPlayer entityplayer) {
		...
		lastWayPoint = new double[] { entityplayer.posX,
				entityplayer.posY - entityplayer.yOffset + 0.7D,
				entityplayer.posZ };
		nowWayPoint = new double[] { entityplayer.posX,
				entityplayer.posY - entityplayer.yOffset + 0.7D,
				entityplayer.posZ };
		wayPoints.clear();
	...
	public boolean checkRoutePoint(World world, EntityPlayer entityplayer) {
		...
		MovingObjectPosition mo = world.rayTraceBlocks_do_do(Vec3
				.createVectorHelper(lastWayPoint[0], lastWayPoint[1],
						lastWayPoint[2]), vec1, false, true);
		if (mo != null) {
			// ポイントの追加
			wayPoints.add(nowWayPoint);
			lastWayPoint = nowWayPoint;
			System.out
					.println(String.format("Add Route(%d).", wayPoints.size()));
	...
	public void checkReRoute(World world, EntityPlayer entityplayer) {
		...
		if (ldd != lastWayPoint) {
			Vec3 vec1 = entityplayer.getPosition(1.0F);
			vec1.yCoord -= entityplayer.yOffset - 0.7D;
			MovingObjectPosition mo = world.rayTraceBlocks_do_do(
					Vec3.createVectorHelper(ldd[0], ldd[1], ldd[2]), vec1,
					false, true);
			if (mo == null) {
				// 視線が通る
				// 以降のリストを削除
				...
				for (int li = 0; li < 3 && (wayPoints.size() >= li + 1); li++) {
					entityRoute.routePos[li] = wayPoints.get(counterCheckWay
							- li);
				}
				lastWayPoint = entityRoute.routePos[0];

				System.out.println(String.format("reRoute %d Check points",
						wayPoints.size()));
			}
		}
	...
	private void drawRECON(Tessellator tessellator, Minecraft lmc,
			GRV_EntityRECON pEntityRECON) {
		// RECONのマーカー
		...
		GLU.gluProject((float) (pEntityRECON.posX - entityRoute.posX),
				...
				GRV_RenderMARKER.matModel, GRV_RenderMARKER.matProjection,
				GRV_RenderMARKER.matViewport, posMarker);
		...
		if (posMarker.get(2) >= 0.0F && posMarker.get(2) <= 1.0F) {
			...
			GL11.glLineWidth(1.5F);
			GL11.glDisable(GL11.GL_TEXTURE_2D);
			GL11.glTranslatef(posMarker.get(0) / zoomx, (float) centerY * 2F
					- posMarker.get(1) / zoomy - 10F, 0.0F);
			...
			lmc.fontRenderer.drawString(lstr, lw,
					(int) (ly - lmc.fontRenderer.FONT_HEIGHT * 1), lcolor);
			lstr = String.format("ENEMY x%02d", pEntityRECON.getCountEnemy());
			lmc.fontRenderer.drawString(lstr, lw,
					(int) (ly - lmc.fontRenderer.FONT_HEIGHT * 2), lcolor);
			lstr = String.format("RECON %3d", pEntityRECON.index + 1);
			lmc.fontRenderer.drawString(lstr, lw,
					(int) (ly - lmc.fontRenderer.FONT_HEIGHT * 3), lcolor);
		} else {
			// マーカーは画面外
