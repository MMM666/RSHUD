...
public class GuiRSHUD_AC extends GRH_GuiRSHUDConfigure {
	...
	public void initGui() {
		super.initGui();
		deg = (new MMM_GuiSlider(50, hwSize + 60, (hhSize - 106) + 0, "Deg+", (float) mod_RSHUD_AC.DegOffset / 36F, 36F, 0.0F)).setStrFormat("%s : %.0f").setDisplayString();
		buttonList.add(deg);
		linea = (new MMM_GuiSlider(51, hwSize + 60, (hhSize - 106) + 24, "Line A", mod_RSHUD_AC.LineAlpha)).setDisplayString();
		buttonList.add(linea);
		linew = (new MMM_GuiSlider(52, hwSize + 60, (hhSize - 106) + 48, "LineWidth", (mod_RSHUD_AC.LineWidth - 0.5F) / 5F, 5F, 0.5F)).setDisplayString();
		buttonList.add(linew);
		String as[] = { "\2474R", "\2472G", "\2471B", "A" };
		float af[] = getRGBA(ColorInt_Normal);

		for (int i = 0; i < 4; i++) {
			colornormal[i] = (new MMM_GuiSlider(10 + i, hwSize - 205, hhSize + 14 + 24 * i, as[i], af[i])).setDisplayString();
		...
		for (int j = 0; j < 4; j++) {
			colorwarning[j] = (new MMM_GuiSlider(20 + j, hwSize - 100, hhSize + 14 + 24 * j, as[j], af[j])).setDisplayString();
		...
		for (int k = 0; k < 4; k++) {
			coloralert[k] = (new MMM_GuiSlider(30 + k, hwSize + 5, hhSize + 14 + 24 * k, as[k], af[k])).setDisplayString();
		...
		for (int l = 0; l < 4; l++) {
			colornumber[l] = (new MMM_GuiSlider(40 + l, hwSize + 110, hhSize + 14 + 24 * l, as[l], af[l])).setDisplayString();
	...
	public void drawScreen(int i, int j, float f) {
		...
		mod_RSHUD_AC.Color_Normal = String.format("%08x", new Object[] { Integer.valueOf(ColorInt_Normal) });
		mod_RSHUD_AC.Color_Warning = String.format("%08x", new Object[] { Integer.valueOf(ColorInt_Warning) });
		mod_RSHUD_AC.Color_Alert = String.format("%08x", new Object[] { Integer.valueOf(ColorInt_Alert) });
		mod_RSHUD_AC.Color_Number = String.format("%08x", new Object[] { Integer.valueOf(ColorInt_Number) });
		String s = "LINE Color";
	...
	public void renderRSHUD(Minecraft minecraft, int i, int j) {
		...
		for (int i3 = -i / 20; i3 < i / 20; i3++) {
			...
			if ((k + i3) % 18 == 0) {
				...
				if (j3 == 0) {
					String s2 = "N";
					minecraft.fontRenderer.drawString(s2, (int) (d / 2D + d2),
							(int) d3, k2);
				}
				
				if (j3 == 9) {
					String s3 = "E";
					minecraft.fontRenderer.drawString(s3, (int) (d / 2D + d2),
							(int) d3, k2);
				}
				
				if (j3 == 18) {
					String s4 = "S";
					minecraft.fontRenderer.drawString(s4, (int) (d / 2D + d2),
							(int) d3, k2);
				}
		...
		s = String.format("%.1f", new Object[] { Double.valueOf(minecraft.thePlayer.posY - 1.6000000000000001D) });
		...
		for (int k3 = -7; k3 < 7; k3++) {
			if (d3 > d1 - 63D && d3 < d1 + 60D && (k3 & 1) == 0) {
				...
				minecraft.fontRenderer.drawString(String.format("%d0", new Object[] { Integer.valueOf(i4) }), (int) d2 - 104, (int) d3, k2);
				minecraft.fontRenderer.drawString(String.format("%d0", new Object[] { Integer.valueOf(i4) }), (int) d2 + 94, (int) d3, k2);
			}
		...
		if (minecraft.thePlayer.posY < 30D) {
			...
			minecraft.fontRenderer
					.drawString(s8, i - minecraft.fontRenderer.getStringWidth(s8) - 10, 100, k4);
		}

		if (minecraft.thePlayer.posY < 16D) {
			String s9 = "Diamond";
			minecraft.fontRenderer
					.drawString(s9, i - minecraft.fontRenderer.getStringWidth(s9) - 10, 110, l4);
		}
		...
		String s11 = String.format("%02d", new Object[] { Integer.valueOf(k5) });
		minecraft.fontRenderer.drawString(s11, 70, j - 12, k2);
		int l5 = (int) ((double) (j5 % 100) / 1.66666665D);
		String s12 = String.format("%02d", new Object[] { Integer.valueOf(l5) });
		...
		if (itemstack != null && itemstack.isItemStackDamageable()) {
			...
			s22 = String.format("%7d", new Object[] { Integer.valueOf(l6) });
		...
		if (itemstack != null
			...
			if (i7 == 0) {
				...
				s23 = String.format("%7d", new Object[] { Integer.valueOf(i7) });
			}
		...
		if (k8 != 0) {
			...
			minecraft.fontRenderer.drawString(String.format("%d0", new Object[] { Integer.valueOf(k8) }), 40, 18, k2);
		}
