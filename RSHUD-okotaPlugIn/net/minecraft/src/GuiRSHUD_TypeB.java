...
public class GuiRSHUD_TypeB extends GRH_GuiRSHUDConfigure {
	...
	public void drawDefaultBackground() {
		drawGradientRect(hwSize + 50, hhSize - 116, hwSize + 170, hhSize - 4,
				0xd0101010, 0xc0101010);
		drawGradientRect(hwSize - 210, hhSize - 16, hwSize + 210, hhSize + 116,
	...
	public void drawScreen(int i, int j, float f) {
		...
		ColorInt_Number = setRGBA(colornumber[0].getSliderValue(),
				colornumber[1].getSliderValue(),
				colornumber[2].getSliderValue(),
				colornumber[3].getSliderValue());
		mod_RSHUD_TypeB.Color_Normal = String.format("%08x",
				new Object[] { Integer.valueOf(ColorInt_Normal) });
		mod_RSHUD_TypeB.Color_Warning = String.format("%08x",
				new Object[] { Integer.valueOf(ColorInt_Warning) });
		mod_RSHUD_TypeB.Color_Alert = String.format("%08x",
				new Object[] { Integer.valueOf(ColorInt_Alert) });
		mod_RSHUD_TypeB.Color_Number = String.format("%08x",
				new Object[] { Integer.valueOf(ColorInt_Number) });
		String s = "LINE Color";
		drawString(fontRenderer, s,
				hwSize - 155 - fontRenderer.getStringWidth(s) / 2, hhSize - 12,
				ColorInt_Normal);
		s = "ORE Alpha";
		int k = ColorInt_Normal / 0x1000000;
		int l = k * 0x1000000 + 0x807050;
		drawString(fontRenderer, s,
				hwSize - 155 - fontRenderer.getStringWidth(s) / 2, hhSize + 2,
				l);
		s = "WARNING";
		drawString(fontRenderer, s,
				hwSize - 55 - fontRenderer.getStringWidth(s) / 2, hhSize + 2,
				ColorInt_Warning);
		s = "ALERT";
		drawString(fontRenderer, s,
				(hwSize + 50) - fontRenderer.getStringWidth(s) / 2, hhSize + 2,
				ColorInt_Alert);
		s = "NUMBER";
		drawString(fontRenderer, s,
				(hwSize + 155) - fontRenderer.getStringWidth(s) / 2,
				hhSize + 2, ColorInt_Number);
		GL11.glEnable(GL11.GL_BLEND);
		s = "WARNING";
		fontRenderer.drawString(s, hwSize - fontRenderer.getStringWidth(s) / 2,
				hhSize - 50, ColorInt_Warning);
		s = "ALERT";
		fontRenderer.drawString(s, hwSize - fontRenderer.getStringWidth(s) / 2,
				hhSize - 70, ColorInt_Alert);
		s = "NUMBER";
		fontRenderer.drawString(s, hwSize - fontRenderer.getStringWidth(s) / 2,
				hhSize - 90, ColorInt_Number);
	...
	public void renderRSHUD(Minecraft minecraft, int i, int j) {
		...
		for (int l = -i / 10; l < i / 10; l++) {
			...
			if ((k + l) % 18 == 0) {
				...
				tessellator.addVertex(d2 + 2D, d4 - ((l & 1) != 1 ? 5D : 3D),
						0.0D);
			}
		...
		d3 = d1
				- 105D
				- ((double) ((EntityPlayer) (entityplayersp)).rotationPitch % 10D)
				* 3D;
		...
		tessellator.addVertex(d2 + 15D, d1, 0.0D);
		tessellator.draw();
		double d5 = ((EntityPlayer) (entityplayersp)).lastTickPosX
				- ((EntityPlayer) (entityplayersp)).posX;
		double d6 = ((EntityPlayer) (entityplayersp)).lastTickPosY
				- ((EntityPlayer) (entityplayersp)).posY;
		double d8 = ((EntityPlayer) (entityplayersp)).lastTickPosZ
				- ((EntityPlayer) (entityplayersp)).posZ;
		...
		for (int i2 = 0; i2 < j / 5; i2++) {
			if (d3 < d1 - 4D || d3 > d1 + 7D) {
				...
				if (d3 > d1 - 100D && d3 < (double) j - 80D) {
					...
					tessellator.addVertex(d2 - ((i2 & 1) != 1 ? 5D : 3D), d3,
							0.0D);
				}
			}
		...
		for (int j2 = 0; j2 < j / 5; j2++) {
			if (d3 < d1 - 4D || d3 > d1 + 7D) {
				...
				if (d3 > d1 - 100D && d3 < (double) j - 60D) {
					tessellator.addVertex(d2, d3, 0.0D);
					tessellator.addVertex(d2 + ((j2 & 1) != 1 ? 5D : 3D), d3,
							0.0D);
				}
			}
		...
		for (int j3 = -i / 10; j3 < i / 10; j3++) {
			...
			if ((k + j3) % 18 == 0) {
				...
				if (l3 == 0) {
					String s2 = "North";
					minecraft.fontRenderer.drawString(s2, ((int) d2 + 2)
							- minecraft.fontRenderer.getStringWidth(s2) / 2,
							(int) d3, k2);
				}

				if (l3 == 9) {
					String s3 = "East";
					minecraft.fontRenderer.drawString(s3, ((int) d2 + 2)
							- minecraft.fontRenderer.getStringWidth(s3) / 2,
							(int) d3, k2);
				}

				if (l3 == 18) {
					String s4 = "South";
					minecraft.fontRenderer.drawString(s4, ((int) d2 + 2)
							- minecraft.fontRenderer.getStringWidth(s4) / 2,
							(int) d3, k2);
				}

				if (l3 == 27) {
					String s5 = "West";
					minecraft.fontRenderer.drawString(s5, ((int) d2 + 2)
							- minecraft.fontRenderer.getStringWidth(s5) / 2,
							(int) d3, k2);
				}
			}
		...
		for (int k3 = -j / 20; k3 < j / 20; k3++) {
			...
			if ((d3 < d1 - 9D || d3 > d1 + 12D) && i4 % 5 == 0) {
				...
				String s6 = String.format("%4d",
						new Object[] { Integer.valueOf(i4) });

				if (d3 > d1 - 100D && d3 < (double) j - 80D) {
					minecraft.fontRenderer.drawString(s6, (int) d2 + 10,
							(int) d3 - 2, k2);
				}
			}
		...
		String s = String.format("%.1f", new Object[] { Double.valueOf(d9) });
		minecraft.fontRenderer.drawString(s, (int) d - 120
				- minecraft.fontRenderer.getStringWidth(s), j / 2 - 2, k2);
		String s1 = "Speed";
		minecraft.fontRenderer.drawString(s1,
				(int) d - minecraft.fontRenderer.getStringWidth(s1) / 2 - 140,
				(int) d1 - 15, k2);
		d3 = 0.0D + (minecraft.thePlayer.posY * 10D) % 10D;
		...
		for (int j4 = -j / 20; j4 < j / 20; j4++) {
			...
			if ((d3 < d1 - 9D || d3 > d1 + 12D) && k4 % 5 == 0) {
				...
				s = String.format("%4d", new Object[] { Integer.valueOf(k4) });

				if (d3 > d1 - 100D && d3 < (double) j - 60D) {
					minecraft.fontRenderer.drawString(s, (int) d2 + 10,
							(int) d3 - 2, k2);
				}
			}
		...
		s = String.format("%.1f",
				new Object[] { Double.valueOf(minecraft.thePlayer.posY) });
		minecraft.fontRenderer.drawString(s, ((int) d + 155)
				- minecraft.fontRenderer.getStringWidth(s), j / 2 - 2, k2);
		String s7 = "Height";
		minecraft.fontRenderer
				.drawString(
						s7,
						((int) d - minecraft.fontRenderer.getStringWidth(s7) / 2) + 135,
						(int) d1 - 15, k2);
		d2 = d;
		d3 = (double) j
				/ 2D
		...
		for (int l4 = -7; l4 < 7; l4++) {
			if (d3 > d1 - 63D && d3 < d1 + 60D && (l4 & 1) == 0) {
				...
				minecraft.fontRenderer.drawString(String.format("%d0",
						new Object[] { Integer.valueOf(j5) }), (int) d2 - 104,
						(int) d3, k2);
				minecraft.fontRenderer.drawString(String.format("%d0",
						new Object[] { Integer.valueOf(j5) }), (int) d2 + 94,
						(int) d3, k2);
			}
		...
		if (minecraft.thePlayer.posY < 64D) {
			String s8 = "Iron";
			minecraft.fontRenderer.drawString(s8,
					i - minecraft.fontRenderer.getStringWidth(s8) - 6,
					(int) d1 - 20, k5);
		}

		if (minecraft.thePlayer.posY < 30D) {
			String s9 = "Gold";
			minecraft.fontRenderer.drawString(s9,
					i - minecraft.fontRenderer.getStringWidth(s9) - 6,
					(int) d1 - 10, l5);
		}

		if (minecraft.thePlayer.posY < 16D) {
			String s10 = "Diamond";
			minecraft.fontRenderer.drawString(s10,
					i - minecraft.fontRenderer.getStringWidth(s10) - 6,
					(int) d1, i6);
		}

		if (minecraft.thePlayer.posY < 12D) {
			String s11 = "Lava";
			minecraft.fontRenderer.drawString(s11,
					i - minecraft.fontRenderer.getStringWidth(s11) - 6,
					(int) d1 + 15, j6);
		}
		...
		if (minecraft.renderViewEntity.isPotionActive(Potion.hunger)) {
			String s19 = "Hunger";
			j7++;
			minecraft.fontRenderer.drawString(s19, 5, 5 + j7 * 11,
					ColorInt_Warning);
		}
		...
		if (k6 % 3 != 1) {
			if (minecraft.renderViewEntity.isPotionActive(Potion.poison)) {
				...
				minecraft.fontRenderer.drawString(s20, (int) d
						- minecraft.fontRenderer.getStringWidth(s20) / 2,
						(int) d1 - 60, ColorInt_Alert);
			}
		...
		if (itemstack != null && itemstack.isItemStackDamageable()) {
			...
			if ((float) itemstack.getItemDamage()
					/ (float) itemstack.getMaxDamage() > 0.9F) {
				l8 = ColorInt_Alert;
			}

			String s23 = (new StringBuilder())
					.append(itemstack.getItem().getStatName()).append(" /")
					.toString();
			minecraft.fontRenderer
					.drawString(s23, ((int) d - minecraft.fontRenderer
							.getStringWidth(s23)) + 25, (int) d1 + 63, l8);
			s23 = String.format("%7d", new Object[] { Integer.valueOf(i8) });
			minecraft.fontRenderer
					.drawString(s23, ((int) d - minecraft.fontRenderer
							.getStringWidth(s23)) + 60, (int) d1 + 63, l8);
		}

		if (itemstack != null
				&& projectorList.containsKey(Integer.valueOf(itemstack.getItem().itemID))) {
			int j8 = 0;
			...
			for (int j9 = 0; j9 < ((EntityPlayer) (entityplayersp)).inventory.mainInventory.length; j9++) {
				...
				if (itemstack != null
						&& list.contains(Integer.valueOf(itemstack.getItem().itemID))) {
			...
			String s24 = (new StringBuilder())
					.append(Item.itemsList[((Integer) list.get(0)).intValue()]
							.getStatName()).append(" /").toString();
			minecraft.fontRenderer
					.drawString(s24, ((int) d - minecraft.fontRenderer
							.getStringWidth(s24)) + 15, (int) d1 + 54, k9);

			if (j8 == 0) {
				s24 = "EMPTY";
			} else {
				s24 = String
						.format("%7d", new Object[] { Integer.valueOf(j8) });
			}

			minecraft.fontRenderer
					.drawString(s24, ((int) d - minecraft.fontRenderer
							.getStringWidth(s24)) + 50, (int) d1 + 54, k9);
		}
