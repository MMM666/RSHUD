...
public class GRH_GuiRSHUD_TypeA extends GRH_GuiRSHUDConfigure {
	...
	public void initGui() {
		...
		// 100, 20, mod_RSHUD.HUDEnable ? "HUD ON" : "HUD OFF"));
	...
	public void drawDefaultBackground() {
		...
		drawGradientRect(hwSize - 170, hhSize - 4, hwSize + 170, hhSize + 116,
	...
	public void drawScreen(int i, int j, float f) {
		...
		s = "WARNING";
		drawString(fontRenderer, s,
				hwSize - fontRenderer.getStringWidth(s) / 2, hhSize + 2,
				ColorInt_Warning);
		s = "ALERT";
		drawString(fontRenderer, s,
				hwSize + 110 - fontRenderer.getStringWidth(s) / 2, hhSize + 2,
	...
	public void renderRSHUD(Minecraft mc, int i, int j) {
		...
		for (int li1 = -i / 10; li1 < i / 10; li1++) {
			...
			if ((liy + li1) % 18 == 0) {
				...
				tessellator.addVertex(ldx, ldy - ((li1 & 1) == 1 ? 3D : 5D),
						0.0D);
			}
		...
		// mc.thePlayer.rotationPitch, (int)(mc.thePlayer.rotationPitch / 5F)));
		tessellator.startDrawing(GL11.GL_LINES);
		...
		for (int li1 = 0; li1 < j / 5; li1++) {
			if ((ldy < ycenter - 4D) || ((ldy > ycenter + 7D))) {
				...
				tessellator.addVertex(ldx, ldy, 0.0D);
				tessellator.addVertex(ldx - ((li1 & 1) == 1 ? 3D : 5D), ldy,
						0.0D);
			}
			ldy += 5D;
		}
		 ...
		 * (double)chunkcoordinates1.posY, (double)chunkcoordinates1.posZ);
		 * GL11.glRotatef(-mcpl.rotationPitch, 1.0F, 0.0F, 0.0F);
		 * GL11.glRotatef(mcpl.rotationYaw, 0.0F, 1.0F, 0.0F);
		 * GL11.glScalef(40F, 40F, 40F); GL11.glTranslated(mcpl.posX -
		 * (double)chunkcoordinates1.posX + 0.5D, mcpl.posY -
		 * (double)chunkcoordinates1.posY + 0.5D, mcpl.posZ -
		 * (double)chunkcoordinates1.posZ + 0.5D);
		 * GL11.glRotatef(-mcpl.rotationYaw, 0.0F, 1.0F, 0.0F);
		 * GL11.glRotatef(mcpl.rotationPitch, 1.0F, 0.0F, 0.0F); //
		 * GLU.gluPerspective(getFOVModifier(f, true), (float)mc.displayWidth /
		 * (float)mc.displayHeight, 0.05F, (256 >>
		 * mc.gameSettings.renderDistance) * 2.0F);
		 * 
		 * tessellator.startDrawing(GL11.GL_LINE_LOOP); //
		 * tessellator.startDrawingQuads(); tessellator.addVertex(0D, 0.3D,
		 * 0.0D); tessellator.addVertex(0.3D, 0D, 0.0D);
		...
		for (int li1 = -i / 10; li1 < i / 10; li1++) {
			...
			if ((liy + li1) % 18 == 0) {
				...
				mc.fontRenderer.drawString(s,
						(int) ldx - mc.fontRenderer.getStringWidth(s) / 2,
						(int) ldy, txtcolor);
			}
			ldx += 5D;
		}
		...
		for (int li1 = -j / 20; li1 < j / 20; li1++) {
			...
			if (((ldy < ycenter - 9D) || (ldy > ycenter + 12D))
					&& (li2 % 5) == 0) {
				ldx = ldy - ycenter;
				ldx = xcenter - 170D - (ldx * ldx * 0.003D);
				s = String.format("%4d", li2);
				mc.fontRenderer.drawString(s, (int) ldx + 10, (int) ldy - 2,
						txtcolor);
			}
			ldy += 10D;
		}
		...
		for (int li1 = -7; li1 < 7; li1++) {
			if (ldy > ycenter - 75D && ldy < ycenter + 65D && ((li1 & 1) == 0)) {
				...
				if (lps > 9)
					lps = 18 - lps;
				mc.fontRenderer.drawString(String.format("%d0", lps),
						(int) ldx - 104, (int) ldy, txtcolor);
				mc.fontRenderer.drawString(String.format("%d0", lps),
						(int) ldx + 94, (int) ldy, txtcolor);
			}
			ldy += 15D;
		}
		...
		if (is != null && is.isItemStackDamageable()) {
			...
			s = (new StringBuilder()).append(is.getItem().getStatName())
					.append(" /").toString();
			mc.fontRenderer.drawString(s,
